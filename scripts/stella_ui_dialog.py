...
from stella_ui.msg import Robot_status
...
#		return (enable, msg)
	...
	def OnConnect(self, event):
		...
		if v == True:
			self.robot_connect_proc = psutil.Popen(['sshpass', '-p', robot_password, 'ssh', robot_id + '@' + robot_ip, '-oStrictHostKeyChecking=no', 'source /opt/ros/noetic/setup.bash && source ~/catkin_ws/devel/setup.bash && export ROS_MASTER_URI=http://' + remote_ip +':11311 && export ROS_HOSTNAME=' + robot_ip + ' && roslaunch stella_bringup stella_robot.launch'], stdout=out, stderr=err)
		elif v == False:
			self.robot_connect_proc = psutil.Popen(['sshpass', '-p', robot_password, 'ssh', robot_id + '@' + robot_ip, '-oStrictHostKeyChecking=no', 'source /opt/ros/noetic/setup.bash && source ~/catkin_ws/devel/setup.bash && export ROS_MASTER_URI=http://' + remote_ip +':11311 && export ROS_HOSTNAME=' + robot_ip + ' && rosnode kill stella_md_node stella_ahrs_node ydlidar_node'], stdout=out, stderr=err)
	...
	def OnSlam(self, event):
		...
		if v == True:
			...
			self.robot_slam_proc = psutil.Popen("gnome-terminal --tab -- roslaunch stella_slam stella_slam.launch && sleep 2 && wmctrl -r 'stella_cartographer.rviz - RViz' -e 0,0,180,630,770",stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)
	...
	def OnNavigation(self, event):
		...
		if v == True:
			self.robot_nav_proc = psutil.Popen("gnome-terminal --tab -- roslaunch stella_navigation stella_navigation.launch map_file:=" + self.path_navigation + "&& sleep 2 && wmctrl -r 'stella_navigation.rviz - RViz' -e 0,0,180,630,770",stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)
		elif v == False:
